...
```{r}
download.file(url = "http://ftp.flybase.org/releases/FB2024_04/precomputed_files/genes/fbgn_NAseq_Uniprot_fb_2024_04.tsv.gz", destfile = "dmel_6_59_DB_identifiers.tsv.gz" )
```

This is a *tab seperated values* flat file. 

If you unzip the file and look at it (excel will work, as will any basic text editor), you will see the first 5 lines of the file  begin with "##". The first four of those lines don't matter very much, but the fifth line has the headers. So you can import this file into R (keep in mind the appropriate delimiter), excluding the first four lines. However, this file is a bit of a mess, so I have made a cleaner version for us to work with as a csv (comma seperated values). However, feel free to learn how to do this yourself! Again you may wish to download a local copy of this file to work with instead of downloading it each time you run your script.

```{r}
Dm_identifiers <- read.csv( file = "https://raw.githubusercontent.com/DworkinLab/Bio720/refs/heads/master/data/dmel_6_59_DB_identifiers.csv",
                            h = TRUE, na.strings = "")
```


This file should have 937210 rows and 9 columns. Which you should check.


Q3A - Create a new object from `Dm_identifiers` called `Dm_FBgn_refseq`, that contains columns "gene_symbol", "FBgn" and "RefSeq_transcripts", and then filter it so that any columns that don't have both a "Fbgn" and RefSeq identifier are removed.
