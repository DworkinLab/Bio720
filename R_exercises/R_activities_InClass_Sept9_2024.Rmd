...
#### for loop
In most other languages, a `for` loop would be fine and you could it like this

First we initialize and pre-allocate memory for a vector to store the data we are going to generate (the sums). In this case I am creating a vector with `NA`, which we will fill with the sums we compute
```{r}
...
All of these achieve the same thing. Although for some reason using colSums stores it as numeric not integers...
```{r}
head(read_sums)
head(read_sums_alt1)
head(read_sums_alt2)
