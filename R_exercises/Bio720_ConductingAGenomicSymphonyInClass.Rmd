...
You have to give the developers of Bioconductor a little love. They have really gone out of their way to provide very powerful *R* libraries, with lots of tutorials. Let's take a look shall we. The bioconductor home page is [here](http://bioconductor.org/). Also the journal F1000Research has a useful hub of [papers and tutorials](https://f1000research.com/gateways/bioconductor)

## A little introduction with Drosophila

With 2418 (as of May 7th, 2026) [packages](https://www.bioconductor.org/packages/release/BiocViews.html#___Software) it is not like I can show you everything. However the Bioinformatic Data Skills book, and the datacamp tutorial will give you a tiny bit of an intro into things.
...
### you will need to install things a bit differently for bioconductors
```{r}
BiocManager::install(c("BSgenome", "GenomicFeatures", "Biostrings", "seqLogo", "txdbmaker", "GenomeInfoDbData")) 
```
...
In terms of installing BioConductor genomes and libraries they have deprecated some functions about 7 years ago. I only mention in because you may see some old code using the `biocLite` function which is now **deprecated**.
...
Use the help from available.genomes to see how you can take a look at the genomes
...
We can take a look at the type of object it is?
```{r}
...
**Question:** How long is the **assembled** genome in base pairs (bp)? How about in mega basepairs (Mb)?
...
Let's take a look at just one arm of one chromosome (chromosome "2R")
...
**Question:** How would you represent this as probabilities instead. Use the help file for alphabetFrequency.
 ...
 - How about if you only wanted frequency of A, C, G or T?
 

## examining multiple bp (dinucleotides, etc)

Similar for dinucleotide, trinucleotide and arbitrary oligonucloetides.

This can be very useful to know about and can speed up your work flow!
...
**Question:** Of all possible homopolymers of length 6 (or greater) which is the most frequent?
...
Transition probabilities from dinucleotides...
```{r}
...
Use the `alphabetFrequency()` to get the frequency of nucleotides on chromosome 2R (chr2R). We only want it for the base
...
Question: How long is this chromosome arm?
...
**Question:**  - Using that same region, how would you find the reverse complement of that sequence? What should it be equivalent to (use the help or vignettes in biostrings to find the functions to help you)


## Finding some simple patterns in our chromosome

Often we are looking through genomes for certain patterns (usually best set up as a kind of regular expression, but with the ambiguous nucleotide characters). For this the "countPattern" function and its family are really useful (check the help). They allow for mismatches as well.

```{r}
seqs <- c("CCCGGG", "CCCCTTTT", "AAATTT", "TAGCT")

n = sapply(seqs, 
           function(x) countPattern(x, dm_chrX ))
...
```{r}
TF_hits_rc <- matchPWM(reverseComplement(pwm),
                       dm_chrX, with.score = TRUE)
