...
This is not really a great way of plotting it at all! But we may wish to at least have both axes on the same scale. Also see the functions `lims` and `coord_cartesian` and those starting with `scale_`
...
Or make your own.. Try working with theme to modify some of the attributes for the plotting theme.
...
Try plotting this same data as both a beeswarm and a violin plot.
...
For a beeswarm, these are two approaches that are relatively straightforward to understand
```{r}
ggplot(data = dll_data_subset, aes(y = tarsus, x = temp, colour = genotype)) +
