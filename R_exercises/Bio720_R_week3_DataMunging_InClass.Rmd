...
First let's check if there is any missing data. You can use the `is.na()` to check each row, and then either use `sum` or `any` to evaluate across the whole object (the `data.frame` in this case). Try this.


More generally we can use the function `anyNA` to do this on the `data.frame`.
...
mean(dll_data$femur)
```
- Since there is some missing data, the `mean()` function defaults to NA, to warn you. This is a useful behaviour. How can you deal with this (look at the help of `mean`)



### What to do with this rows with missing data
We need to decide what to do with the missing data. If you have lots of data, and do not mind removing all cases (rows) that contain **any** missing data then `na.omit` is a useful solution. We will do that here, but it is often not a generally good way to do it, if you have lots of missing data, but in lots of different variables. 
...
This has caused us to remove XX observations. It may be useful to instead look at where the missing data is contained and maybe make a subsetted data set based on that. There are numerous options about how `R` should handle operations if an `NA` is found. You can look at the help under `?NA`. 
...
We can first ask if any rows are duplicates. This provides a Boolean (`TRUE` or `FALSE`).  Take a look by looking at the first and last (using `head` and `tail`) values using `duplicated`

How many duplicated rows do we have in this data set?

`R` has a convenience function for the above, `anyDuplicated()`

How many observations have "duplicated values` (i.e. rows are totally identical)? hint, use the index! You may find the function `which` very useful here. as well, although there are other approaches.


You will see in this case there is XXX "duplicated. Which in this case, is because there is so much missing data for this observation, and only one variable (SCT) had actually data for a discrete value. So it is likely real, and not an artificial duplicate.

However let's say an accident happened in generating the data set and a few rows were duplicated. I am going to use the `sample()` function to extract 5 random rows and add them to a new data set `dll_data2`.
...
So what should we do? We can use the `unique()` to remove these duplicated rows if we know for certain that they do not belong in the data set (i.e. they are duplicated in error, not just because the data set could actually have duplicated rows).


If you remove these, how many observations are you left with?

```{r, eval = FALSE}
dll_data_unique <- ___
```

Let's just do a bit of clean-up of our environment
...
An alternative is to use the `subset` function. Please use this to make a data frame (`dll_data_Dll`) for the *Dll* factor level of genotype and check what has happened to the *wt* factor level. 


```{r, eval = FALSE}
dll_data_Dll <- subset(x = ___, ___ )
```
...
How would you use this to filter to generate a new data set with just `c("line-Sam", "line-1")`?
...
```{r, eval = FALSE}
line_names2 <- strsplit(x = ___, split = ___)
head(line_names2)
...
Say we wanted to call the "sam" line by its proper name "Samarkand". How could we do this? We can use the `sub` function to substitute one string for another. replace "sam" with "Samarkand" using `sub`

```{r, eval = FALSE}
line_names3 <- sub(pattern  = ___, 
                   replacement = ___, 
                   x = ___()
```
