...
With 2083 (as of April 13th, 2023) [packages](https://www.bioconductor.org/packages/release/BiocViews.html#___Software) it is not like I can show you everything. However the Bioinformatic Data Skills book, and the datacamp tutorial will give you a tiny bit of an intro into things.
...
```{r}
methods("length")  # length is a "generic function". Depending on what the class of the object it is being used on, it may behave differently than you expect

showMethods("names") # same idea for S4 objects. Slots are accessed via "@" not "$"
```
...
**Questions:** How would you represent this as probabilities instead. Use the help file for alphabetFrequency. 
...
Often we are looking through genomes for certain patterns (usually best set up as a kind of regular expression, but with the ambiguous nucleotide characters)
...
THis is a collection of known transcription factor (TF) binding sites in the Drosophila genome for the HNF transcription factor that is already in bioconductor (although you may have to install)
...
Oftem the Transcription factor binding sites (TFBS) are upstream of the coding sequence of genes (although in Drosophila, many are intronic as well). Let's download the transcripts (and their genomic co-ordinates/positions) and use this to extract the upstream sequences.
...
class(up10000seqs) # as we would hope, a DNAStringSet
```
...
I also recommend looking at the tools in the [txdbmaker](https://www.bioconductor.org/packages/release/bioc/html/txdbmaker.html) library, which provides a great deal of additional functionality.
