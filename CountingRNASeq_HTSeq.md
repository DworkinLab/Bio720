...
For today, we can actually house all of these smaller files in your space. However, normally it would be best to keep them in scratch
...
Now copy the reads over to the directory `reads`

```
cp /tbd/*.fastq  ~/drosophila_rnaseq/reads
```

## Tools we are using
 For today we will take a basic approach and will map reads using one of the well known splice aware aligners (tophat), and use HTseq to get counts. Thankfully Brian has already installed of these tools on his cluster.
...
Running this block of code may take a bit of time (especially the bowtie-build command). If you don’t want to wait for it, we’ve included some pre-computed reference files for you.

## Mapping reads
Now we’re ready for the first step: mapping our RNA-seq reads to the genome. We will use tophat+bowtie1, which together are a splicing-aware read aligner. The raw sequencing reads are in /tbd. Feel free to take a look at how we’ve named and organized the files:
...
do
    ...
    python -m htseq-count --format=bam --stranded=no --order=pos ~/drosophila_rnaseq/mapped/${sample}/accepted_hits.bam ${annotation} > ~/drosophila_rnaseq/counts/${sample}_htseq_counts.txt
done
```
