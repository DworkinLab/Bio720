...
To install for base R () you can use the:
...
BiocManager::install("TxDb.Dmelanogaster.UCSC.dm6.ensGene", force = TRUE) # Drosophila
...
avail[grep("xlaevis", avail)] # which ones have Drosophila melanogaster.
```
...
setwd("/Users/iandworkin/Dropbox/macBook_HD/TeachingAndLectures/Bio722/Tutorial_Feb16/salmon_counts")
...
I can use the file names directly to extract the information

```{r}
sample_names_alt <- gsub("\\quant.sf", "", quant_files)

experimental_data <- stringr::str_split(sample_names_alt, pattern = "_", simplify = TRUE)
```
...
load.model <- formula(~ lane + tissue + temperature + food)
```
...
```{r}
for_pca_rlog <- rlog(test_lane_effects2, 
                blind = FALSE)
dim(for_pca)

for_pca_vst <- vst(test_lane_effects2, 
                blind = FALSE)
```
...
```{r}
meanSdPlot(txi$counts, ranks = FALSE)
```


```{r}
meanSdPlot(log2((txi$counts) + 1), ranks = FALSE)
```

## Can we get RNASeqQC working
```{r}
mean_sd_plot(for_pca_vst)

mean_sd_plot(for_pca_rlog)
```


```{r lane effects, echo=TRUE}
plotPCA(for_pca_vst, 
        intgroup=c("lane", "food"),
        pcsToUse = 1:2,
        ntop = 10000) 
```
...
plotDispEsts(test_tissue_effects2,
             legend = F)
```
Not much different. A few outlie rs though, and it may be worth later going in and checking these.
