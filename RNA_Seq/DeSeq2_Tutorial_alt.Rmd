...
`/2/scratch/Bio722/salmon_counts`
...
```{bash, eval = FALSE}
cd /2/scratch/Bio722/salmon_counts
...
setwd("/Users/iandworkin/Dropbox/macBook_HD/TeachingAndLectures/Bio722/Tutorial_Feb16/")
...
This can be hard to look at. So let's do this ourselves

```{r}
assay(for_pca)[1:10, 1:10]

how_many_variable_genes <- 2000
rlog_counts <- assay(for_pca)

variance_by_genes <- rowVars(rlog_counts)
hist(variance_by_genes)

# select the "n" most variable genes 

dummy_variable_genes <- order(variance_by_genes, decreasing = TRUE)[seq_len(min(how_many_variable_genes, 
        length(variance_by_genes)))]


filtered_genes <- rlog_counts[dummy_variable_genes, ]

pca_out <- prcomp(t(filtered_genes))
summary(pca_out)
```
...
pc_out_design <- data.frame(rna.design, pca_out$x)
```

Nowe we can plot it anyway we want!!!!


```{r}
ggplot(pc_out_design, aes(x = PC3, y = PC4, colour = tissue, shape = food:temperature)) +
...
```{r}
ggplot(pc_out_design, aes(x = PC1, y = PC2, colour = tissue, shape = food:temperature, label = sample)) +
  xlim(-50, 50) + ylim(-50, 50) +
  geom_label(size = 2.5, alpha = 0.5, position = position_jitter(height = 10, width = 4)) +
  theme_light()
```
uh-oh, some samples seem like they were mislabeled.

Grouping with wings are:
samw_17C_gen_fed_R1
samw_17C_gen_fed_R3


Grouping with genitals are:
samw_17C_wings_fed_R1
samw_17C_wings_starved_R2
...
Let's decide what genes to look at for wings and genital discs.

```{r}
temp_counts <- assay(all.data)

temp_counts[rownames(temp_counts) == "FBgn0003975",] #vg
temp_counts[rownames(temp_counts) == "FBgn0267978",] # ap
temp_counts[rownames(temp_counts) == "FBgn0000015",]# Abd-B
...
```{r re_order}
# reorder
tissue_results <- tissue_results[order(abs(tissue_results$stat), decreasing = TRUE),]
...
plotMA(tissue_results_shrunk, ylim =c(-5, 5))
...
mcols(temp_results_wing)$description

summary(temp_results_wing)

plotMA(temp_results_wing, 
       ylim =c(-10, 10))


temp_results_wing <- temp_results_wing [order(temp_results_wing $stat, decreasing = TRUE),]

head(temp_results_wing)
...
plotMA(temp_wing_results_shrunk, ylim =c(-5, 5))
